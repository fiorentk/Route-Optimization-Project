...
def calculate_distance(row1, row2):
    ...
    return round(distance(coords_1, coords_2).m, 2)

# Function to create a distance matrix that also includes connote_code
def create_distance_matrix_with_codes(df):
    distance_matrix = []
    ...
    for i in range(len(df)):
        ...
        for j in range(len(df)):
            distance_row.append({
                'distance': calculate_distance(df.iloc[i], df.iloc[j]), 
...
# Update the create_distance_map function to use the new matrix with connote codes
# Function to create a distance map with the kantor coordinates added
def create_distance_map_with_kantor(df, df_kantor):
    ...
    for courier in couriers:
        for date in dates:
            ...
            distance_matrix = create_distance_matrix_with_codes(extended_df)
...
def get_optimal_route_with_codes(df, km_per_liter_fuel = 40, fuel_price_per_liter=13000):
    ...
    for i in range(len(df)):
        ...
        fuel_savings = round((distance_difference/1000)/ km_per_liter_fuel,2)  # in milliliters
        cost_savings = round(fuel_savings * fuel_price_per_liter)  # Convert cost based on price per ml
